    ...
    public function getAttendeeAvailability()
    {
        if (array_key_exists("attendeeAvailability", $this->_propDict)) {
            if (is_a($this->_propDict["attendeeAvailability"], "\Microsoft\Graph\Model\AttendeeAvailability") || is_null($this->_propDict["attendeeAvailability"])) {
				...
				/* jpoon start */
				if (is_array($this->_propDict["attendeeAvailability"])) {
					$attendees = [];
					foreach( $this->_propDict["attendeeAvailability"] as $attendee ) {
						array_push($attendees, new AttendeeAvailability($attendee) );
					}
					return $attendees;
				} else {
				/* jpoon end */		
    ...
    public function getLocations()
    {
        if (array_key_exists("locations", $this->_propDict)) {
            if (is_a($this->_propDict["locations"], "\Microsoft\Graph\Model\Location") || is_null($this->_propDict["locations"])) {
				...
				/* jpoon start */
				if (is_array($this->_propDict["locations"])) {
					$locations = [];
					foreach( $this->_propDict["locations"] as $location ) {
						array_push($locations, new Location($location) );
					}
					return $locations;
				} else {
				/* jpoon end */					
